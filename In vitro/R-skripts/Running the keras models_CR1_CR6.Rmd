...
model <- load_model_hdf5("Distinguish CR1vCR6 clones with feature values_in_silico.h5") 


# here I read in the right training data, as I need to scale the data set with their mean and sd for the right prediction
cloneA_train <- read.csv("../Data/Data for model set-up/Train_CR1_both_sep.csv")
cloneB_train <- read.csv("../Data/Data for model set-up/Train_CR6_both_sep.csv")
...
df <- select(df,c(-Time,-Raw.Centroid.X,-Raw.Centroid.Y,-Flow.Speed))
...
#Comparing the predicted data with the real data
    ...
    freq_dat <- data.frame(pred_cloneA_freq=length(pred$pred_ID[pred$pred_ID==1])/length(pred$pred_ID),
                        ...
                        clone_A="CR1",clone_B="CR6",clone_A_freq=1-cloneB_freq,
                        clone_B_freq=cloneB_freq, samples=nrow(df))
    
freq_dat_all <- rbind(freq_dat_all,freq_dat)
}
##################################
#Now as well the test data for CR1 and CR6
setwd("../../Data for model set-up")
df <- read.csv("Test_CR1_both_sep.csv")
df <- select(df,c(-X,-ID))
...
#Comparing the predicted data with the real data
    ...
    freq_dat <- data.frame(pred_cloneA_freq=length(pred$pred_ID[pred$pred_ID==1])/length(pred$pred_ID),
                           pred_cloneB_freq=length(pred$pred_ID[pred$pred_ID==2])/length(pred$pred_ID),
                        clone_A="CR1",clone_B="CR6",clone_A_freq=1,
                        clone_B_freq=0, samples=nrow(df))
    
freq_dat_all <- rbind(freq_dat_all,freq_dat)

##########################################################################################
df <- read.csv("Test_CR6_both_sep.csv")
df <- select(df,c(-X,-ID))
...
#Comparing the predicted data with the real data
    ...
    freq_dat <- data.frame(pred_cloneA_freq=length(pred$pred_ID[pred$pred_ID==1])/length(pred$pred_ID),
                           pred_cloneB_freq=length(pred$pred_ID[pred$pred_ID==2])/length(pred$pred_ID),
                        clone_A="CR1",clone_B="CR6",clone_A_freq=0,
...
write.csv(freq_dat_all,"Predicted frequencies_CR1_CR6_both_sep.csv")
```
